...
export function readTask(taskFilePath: string) {
    try {
        ...
        // use a ternary operator to handle non-Error types being returned
        const message = error instanceof Error ? error.message : String(error);
        console.error(`Error reading task file ${taskFilePath}: ${message}`);
        throw new Error(`Error reading task file ${taskFilePath}: ${message}`);
    }
